...
int main(int argc, char** argv){
    ...
    if (cmd == "version"){ std::cout << "5.0.0\\n"; return 0; }
    if (cmd == "selfcheck"){
        Logger log;
        log.log(LogLevel::INFO, "Starting selfcheck");
        // parse sample CSV embedded in repo structure (if present)
        std::ifstream f("examples/sample.csv");
        if (!f){ log.log(LogLevel::WARN, "examples/sample.csv not found; continuing"); }
        else {
            SlidingWindow w(10);
            std::string line, err; bool first=true;
            while (std::getline(f, line)){
                if (first){ first=false; continue; }
                auto r = parse_csv_line(line, err);
                if (r) w.add(*r);
            }
            log.log(LogLevel::INFO, "Selfcheck parsed sample.csv");
        }
        log.log(LogLevel::INFO, "Selfcheck OK");
        return 0;
    }
    ...
    if (cmd == "bench"){
        ...
        for (int k=0;k<iters;k++){
            ...
            bool first=true;
            while (std::getline(f, line)){
                if (fmt=="csv" && first){ first=false; continue; }
                lines++;
    ...
    std::string outfmt = "text"; std::string out_file;
    bool summary = false;
    std::map<std::string,long long> fail_on;
    ...
    for (int i=2;i<argc;i++){
        ...
        else if (a=="--out-file" && i+1<argc) out_file = argv[++i];
        else if (a=="--summary") summary = true;
        else if (a=="--fail-on" && i+1<argc){
            std::string spec = argv[++i];
            auto pos = spec.find('=');
            if (pos==std::string::npos){ std::cerr << "invalid --fail-on spec: " << spec << "\\n"; return 1; }
            std::string type = spec.substr(0,pos);
            long long v = std::stoll(spec.substr(pos+1));
            fail_on[type] = v;
        }
        else if (a=="-h" || a=="--help"){ help(argv[0]); return 0; }
    ...
    OutputConfig ocfg; ocfg.format = outfmt; if (!out_file.empty()) ocfg.file = out_file;
    ...
    if (summary){
        std::cout << "=== Summary ===\\n";
        for (auto& kv: counters) std::cout << kv.first << ": " << kv.second << "\\n";
    }

    // Fail-on gates
    for (auto& kv: fail_on){
        auto it = counters.find(kv.first);
        long long count = (it==counters.end()) ? 0 : it->second;
        if (count >= kv.second){
            std::cerr << "Fail-on triggered for " << kv.first << " >= " << kv.second << "\\n";
            return 3;
        }
    }
