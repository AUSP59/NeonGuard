...
struct Thresholds {
    ...
    int ddos_events = 100;      // total events hitting the dst in window
    int ddos_unique = 20;       // unique sources
...
inline void detect_portscan(const SlidingWindow& w, const LogRecord& r, const Thresholds& t, std::vector<Finding>& out){
    int attempts = w.port_attempts(r.src_ip, r.dst_ip, r.dst_port);
    if (attempts >= t.portscan_attempts){
        out.push_back({ "portscan", r.src_ip + "->" + r.dst_ip + ":" + std::to_string(r.dst_port), r.ts,
                        "attempts=" + std::to_string(attempts) });
    }
}

inline void detect_ddos(const SlidingWindow& w, const LogRecord& r, const Thresholds& t, std::vector<Finding>& out){
    // naive: unique attackers threshold
