...
int main(){
  ...
  // Simulate portscan from 1.1.1.1 to 2.2.2.2 on ports 1,2,3
  long long ts = 1000;
  for (int p=1;p<=3;p++){
  ...
  bool got_scan=false; for (auto& f: out) if (f.type=="portscan") got_scan=true;
  assert(got_scan);

  // DDoS: two unique attackers to same dst within window
  out.clear();
  ...
  bool got_ddos=false; for (auto& f: out) if (f.type=="ddos") got_ddos=true;
  assert(got_ddos);
  return 0;
}
