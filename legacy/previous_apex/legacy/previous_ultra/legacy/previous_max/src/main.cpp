...
static void help(const char* exe){
    std::cout << "neonsec " << "2.0.0" << "\n"
              << "Usage:\n"
              << "  " << exe << " analyze --input <file|-> --format csv|ndjson [--window SEC] [--rules rules.json]\n"
              << "         [--metrics :PORT] [--plugin <path>]... [--threads N]\n"
              << "Examples:\n"
              << "  " << exe << " analyze --input examples/sample.csv --format csv --window 60 --metrics :9100\n";
}
...
int main(int argc, char** argv){
    ...
    for (int i=2;i<argc;i++){
        ...
        else if (a=="--metrics" && i+1<argc) { std::string v = argv[++i]; auto pos=v.rfind(':'); metrics_port = std::stoi(v.substr(pos+1)); }
    ...
    MetricsServer ms; if (metrics_port>0) ms.start(metrics_port);
